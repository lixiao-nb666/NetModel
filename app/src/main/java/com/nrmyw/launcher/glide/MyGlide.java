...
//    }
    ...
    public void getBitmapWAndH(final Context context, final String url, final GetBitmapImp getBitmapImp) {
        Glide.with(context).load(url).asBitmap().into(new SimpleTarget<Bitmap>() {
            @Override
            public void onResourceReady(Bitmap resource, GlideAnimation<? super Bitmap> glideAnimation) {
                ...
                if (getBitmapImp != null) {
                    getBitmapImp.getBitMapWAndH(w,h);
                }
            }
        }); //方法中设置asBitmap可以设置回调类型
    }
    ...
    public interface GetBitmapImp {
        ...
        public void getBitMapWAndH(int w,int h);
    }
}
